 ...
 * <p/>
 * Copyright {2015} {SpringXu} <hquspring@gmail.com>
 * <p/>
...
/**
 * Created by SpringXu on 15/8/20.
 */
public class ImageUtils {
    ...
    private final static void Log(String log) {
        if (ISDEBUG) {
            Log.d("", log);
        }
    }
    ...
    public static Bitmap loadHugeBitmapFromDrawable(Resources resources, int drawableId, int imgH, int imgW) {
        ...
        Log("width:" + width + " height:" + height + " mimeType:" + mimeType);
        ...
        Log("memory size:" + getBitmapSizeInMemory(width / sampleSize, height / sampleSize));
        Bitmap bitmap = BitmapFactory.decodeResource(resources, drawableId, options);
        Log("w=" + bitmap.getWidth() + " h=" + bitmap.getHeight() + " bitmap size:" + bitmap.getRowBytes() * bitmap.getHeight());
        return bitmap;
    }
    ...
    public static Bitmap loadHugeBitmapFromSDCard(String imgPath, int imgH, int imgW) {
        ...
        Log("memory size:" + getBitmapSizeInMemory(width / sampleSize, height / sampleSize));
        Bitmap bitmap = decodeFile(imgPath, options);
//        Bitmap bitmap = BitmapFactory.decodeFile(imgPath, vinciOptions);
    ...
    public static Bitmap decodeFile(String pathName, BitmapFactory.Options opts) {
        ...
        FlushedInputStream fs = null;
        try {
            stream = new FileInputStream(pathName);
            fs = new FlushedInputStream(stream);
            bm = BitmapFactory.decodeStream(fs, null, opts);
        } catch (Exception e) {
            /*  do nothing.
    ...
    public static String getCatchTypeByUri(String uri) {
        if (TextUtils.isEmpty(uri)) {
            return null;
        }

        if (uri.startsWith(Const.IMAGE_TYPE_RESOURCE)) {
            return CatcherType.RESOURCE;
        }

        if (uri.startsWith(Const.IMAGE_TYPE_SDCARD)) {
            return CatcherType.SDCARD;
        }

        if (uri.startsWith(Const.IMAGE_TYPE_PROVIDER)) {
    ...
    public static String getCatchUrlByUri(String uri) {
        if (TextUtils.isEmpty(uri)) {
            return null;
        }

        if (uri.startsWith(Const.IMAGE_TYPE_RESOURCE)) {
            return uri.substring(Const.IMAGE_TYPE_RESOURCE.length());
        }

        if (uri.startsWith(Const.IMAGE_TYPE_SDCARD)) {
            return uri.substring(Const.IMAGE_TYPE_SDCARD.length());
        }

        if (uri.startsWith(Const.IMAGE_TYPE_PROVIDER)) {
            return uri.substring(Const.IMAGE_TYPE_PROVIDER.length());
        }
     ...
     * @return
     */
    public static String getCacheKey(String url, int w, int h) {
    ...
    public static int getExifOrientation(String path) {
        ...
        try {
            ...
            MLog.e("path=" + path + " error:" + e.getMessage());
